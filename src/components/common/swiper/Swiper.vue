...
<script>
  export default {
    ...
    props: {
      ...
      showIndicator: {
        type: Boolean,
        default: true        //手写的时候，把default打成了fefault,导致轮播图的indicator没有显示，排查了1个小时，值得铭记。
      }
    },
    data() {
      return {
    ...
    methods: {
      ...
      *校验正确的位置
      ...
      setTransform: function(position){
        ...
        //ES6中的模板字符串 ``：1.可以通过${}包含变量；2.支持字符串换行
        this.swiperStyle.transform = `translate3d(${position}px, 0, 0)`;
        this.swiperStyle['-webkit-transform'] = `tranlate3d(${position}px),0,0`;
